...
Look at the class of `expr`. To use `dplyr` tools we'll need to create a data frame. We can acheive this with the `data.frame()` function.

*** =instructions

- Create a new data frame called `expr_df` from `expr` using the `data.frame()` function.
- Combine `fdat` and `expr_df` into a new data frame called `exprf_df` again using the `data.frame()` function.
- Click 'Submit Answer'.

*Note: We set the global option `stringsAsFactors` to `FALSE` to avoid using the `factor` data type which we will not discuss.*
...
*** =sample_code
```{r}
# Cast expr into a data frame called expr_df

# Combine fdat and expr_df into exprf_df
...
# Combined fdat and expr_df into exprf_df
