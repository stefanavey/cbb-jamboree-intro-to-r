...
- Add a `geom_line()` layer to the existing plot and specify the `group` aesthetic in the `geom_line()` call to be `Replicate`.
...
- Modify the `group` aesthetic in `geom_line()` to depend on both `Replicate` and `Treatment` using the `interaction()` function (`interaction(Replicate, Treatment)`).
