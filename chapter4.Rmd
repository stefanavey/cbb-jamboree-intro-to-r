...
# Combine features and expr_df into a data frame named expr_df
expr_df <- data.frame(features, expr_df)
...
success_msg("Nice job making that visualization! Let's spice it up a bit...")
```
...
test_ggplot(check_data = TRUE, data_fail_msg = "Make sure to provide the filtered data",
            check_aes = TRUE, aes_fail_msg = "Did you specify the `x` and `y` aesthetics in `aes()` to plot `Time` versus `expression`?",
            check_geom = TRUE, geom_fail_msg = "Did you keep the `geom_point()` layer and add a new `geom_line()` layer?")
...
Make a plot to see how *MX1* behaves in each `Treatment`.

*** =instructions

Create a plot similar to the previous plot except:
...
success_msg("Lookin' good! I'd publish that!")
```
