...
- Plot the expression of MX1 over time during the 'Brevig' infection
time course.
  ...
  - Then call ggplot on this new variable specifying the data and aeshetics (x and y) and add a geom_point layer.
...
load(url("http://s3.amazonaws.com/assets.datacamp.com/production/course_1843/datasets/data.RData"))
options(stringsAsFactors = FALSE)
```
...
test_ggplot(all_fail_msg = "Don't forget to make the plot! If you store the ggplot call in a variable `var` you can use `plot(var)` to make the plot",
            check_data = TRUE, check_aes = TRUE, check_geom = TRUE)
