...
Currently our data is stored in 3 variables, `expr`, `features`, and `phenotypes`. Eventually we want to combine all of these into a single tidy data frame.

To begin, we can combine the information on the features with the expression data. Each row of `features` gives additional information on the gene expression probe while each row of `expr` gives the expression levels of each probe in different samples.
...
- Pass `features` and `expr` to the [`data.frame()`](http://www.rdocumentation.org/packages/base/functions/data.frame) function and assign this new data frame to a variable named `expr_df`.
- Click 'Submit Answer'.
...
```{r}
# Combine features and expr into a data frame named expr_df
expr_df <- data.frame(features, expr)
```
...
- Use [`gather()`](http://www.rdocumentation.org/packages/tidyr/functions/gather) on `expr_df` specifying the key as 'geo_accession' and the value as 'expression' and save this in a variable called `dat`
...
# Gather expression columns of expr_df and print structure of dat
dat <- gather(data = expr_df, key = geo_accession,
...
Finally, you want to add phenotype data in `phenotypes` to `dat` so you have all the information describing the geo_accession (i.e. sample) in each row.
...
- Use [`full_join()`](http://www.rdocumentation.org/packages/dplyr/functions/full_join) to join `dat` and `phenotypes` by the "geo_accession" column and save the result in a new variabled called `data`
...
# Join dat and phenotypes
data <- full_join(dat, phenotypes, by = "geo_accession")
```
...
You will start exploring by plotting the expression of one gene over time during influenza infection.
...
- Plot the `expression` of the gene *MX1* over `Time` during infection with the *Brevig* strain.
  ...
  - First [`filter()`](http://www.rdocumentation.org/packages/dplyr/functions/filter) `data` to observations where the `Symbol` is "MX1" and the `Treatment` is "Brevig" and save this in a new variable.
...
# Note we are doing the same steps here using piping (%>%)
data <- data.frame(features, expr) %>%
  gather(key = geo_accession, value = expression,
         starts_with("GSM")) %>%
  full_join(phenotypes, by = "geo_accession")
...
success_msg("Nice job making that visualization! You can see that the expression of *MX1* changes dramatically over the time course. That's because *MX1* is an antiviral response gene that gets activated when a cell is infected. Let's spice it up a bit...")
```
...
Make a plot to see how *MX1* behaves for each `Treatment`.
...
############################################################
# Filter the data
plotData <- data %>% filter(Symbol == "MX1")
...
success_msg("Lookin' good! You can use the buttons on the plot to go back and forth between multiple plots. How does the `Brevig` strain compare to the other strains (`Cal`, `NC`, `Tx`)? `AlaF` is a mock infection while `Control` is no infection. Does this make sense from the plot?")
```
--- type:NormalExercise  key:ae47d50b6a
## Data Interrogation

**TODO**: Add something here on summarizing data (using split-apply-combine methodology).
