...
So how to do this in R? Well, the function [`head()`](http://www.rdocumentation.org/packages/utils/functions/head) enables you to show the first observations of a data frame (Similarly [`tail()`](http://www.rdocumentation.org/packages/utils/functions/head) prints out the last observations in your data set).
...
- Call [`summary()`](http://www.rdocumentation.org/packages/utils/functions/summary) on the [`mtcars`](http://www.rdocumentation.org/packages/datasets/functions/mtcars) data set to summarize each column.


*** =hint
`head(mtcars)` will show the first observations of the `mtcars` data frame.
`summary(mtcars)` will summarize each column of the `mtcars` data frame.
...
test_function("summary", "x", incorrect_msg = "Have you correctly passed `mtcars` to the `summary()` function?")
test_output_contains("summary(mtcars)", incorrect_msg = "Simply print out the result of the `summary()` call, no need to assign it to a new variable.")
...
*** =instructions
Slice the `mtcars` data frame to extract the 10^th^ row for the `mpg` and `carb` columns.
