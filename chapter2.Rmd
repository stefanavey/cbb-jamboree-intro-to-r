...
*** =hint
You need to identify what column you are joining *by*, and in this case, note that `dat2` and `dat3` don't have the same column name for what you want to join by (the letters of the alphabet). Check out the sample code to see what to do in this case, using `dplyr::full_join()` instead of the `dplyr::inner_join()` function.
...
## Your task:
## Combine `dat2` with `dat3` via a full join
dplyr::full_join(dat2, dat3, by = c("id" = "ALPHABET"))
```
...
Multiple filters can be done simultaneously by specifying multiple conditionals to `dplyr::filter()`.
