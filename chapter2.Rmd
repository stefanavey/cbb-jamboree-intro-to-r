...
--- type:NormalExercise xp:50 skills:1 key:3db54e3447
...
--- type:NormalExercise lang:r xp:50 skills:1,4 key:c24bed01fc
## Some (Un)Tidy Data
...
```{r}
## Untidy data example
names <- c("John Smith", "Jane Doe", "Mary Johnson")
                                        
untidy_df <- data.frame(names,
                        treatmenta = c(NA, 4, 6),
                        treatmentb = c(18, 1, 7))
untidy_df
...
--- type:NormalExercise lang:r xp:50 skills:1,4 key:489128dafe
## A Tidy Sample
...
names <- rep(c("Jane Doe", "John Smith", "Mary Johnson"), each = 2)
treatment <- rep(c("a", "b"), 3)
value <- c(4, 1, NA, 18, 6, 7))

tidy_df <- data.frame(names, treatment, value)
...
*** =instructions
- What do the first few and last few rows of the data look like?
...
*** =sct
```{r}
success_msg("If you could answer all those questions, great job. Head over to the next exercise!")
...
## the data by the variable `income`
pew_df
...
Luckily for us, the package `dplyr` has a family of *join* functions that make the task of gluing data back together easier than ever. A swiss army knife of joins are available for your use - see your [cheat sheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf). Check out the sample code at right to see a couple of the joins in action.


*** =instructions
Use a left join to combine `dat2` with `dat3`, and save the result as `dat4`

*** =hint
You need to identify what column you are joining *by*, and in this case, note that `dat2` and `dat3` don't have the same column name for what you want to join by (the letters of the alphabet). Check out the sample code to see what to do in this case, using `dplyr::left_join()` instead of the `dplyr::full_join()` function.
...
## Your task:
## Combine `dat2` with `dat3` via a left join
...
## Your task:
## Combine `dat2` with `dat3` via a left join
dat4 <- dplyr::left_join(dat2, dat3, by = c("id" = "ALPHABET"))
```
...
Sometimes, we only want to look at the most interesting data in the world. To do that, we need to know how to *filter* our dataset. Thankfully, we have a handy `dplyr::filter()` function that does just that! 
...
To do this, we use *conditionals*, such as less-than (`<`), greater-than (`>`), equals (`==`), or not-equals (`!=`) which result in *logical* (boolean) values. Then, our `dplyr::filter()` function will only keep what would evaluate to `TRUE` in our comparison. 


For more advanced filters, we could also combine them using and (`&`) and or (`|`) - we won't get into those here, but it's good to keep in mind for your applications. (Alternately, you can string together multiple individual filters as well)


*** =instructions
Filter the `iris` dataset such that it only contains flowers whose petal width is less than `0.2`, and save the result as `iris_filtered`

*** =hint
Remember, `dplyr::filter()` works such that it is `dplyr::filter(data, conditional)`. Your conditional should be formatted as *Column Name* < 0.2. Inspect the `iris` data to make sure you're using the correct column.
...
df <- data.frame(letters = c("a", "b", "c", "d"),
                 value = c(1, 2, 10, 5))
df$value == 10                  # Note the TRUE in position 3
...
## Filter `iris` such that it only contains flowers whose petal width 
## is less than 0.2 
iris_filtered <- dplyr::filter(iris, Petal.Width < 0.2)
