...
test_or(
    ...
    test_output_contains("pew[1:2,]"),
    test_output_contains("pew[1:3,]"),
    test_output_contains("pew[1:4,]"),
    test_output_contains("pew[1:5,]"),
    test_output_contains("pew[1:6,]"), 
    incorrect_msg = "Did you look at the first few (no more than 6) lines of `pew`?")
nr <- nrow(pew)
test_or(
    test_output_contains("tail(pew)"),
    test_output_contains("pew[(nr-1):nr,]"),
    test_output_contains("pew[(nr-2):nr,]"),
    test_output_contains("pew[(nr-3):nr,]"),
    test_output_contains("pew[(nr-4):nr,]"),
    test_output_contains("pew[(nr-5):nr,]"), 
    incorrect_msg = "Did you look at the last few (no more than six) lines of `pew`?")
test_or(
...
Back to our data..what we want to do is *gather* our columns into a single variable - income. To accomplish this, let's use the [`tidyr::gather()`](http://www.rdocumentation.org/packages/tidyr/functions/gather) function from the `tidyr` package. A quick example with the built-in `iris` dataset is shown in the Sample Code.
...
*** =hint
See the sample code and your data wrangling cheat sheet for an example of how to use [`tidyr::gather()`](http://www.rdocumentation.org/packages/tidyr/functions/gather). Note that the opposite of this is [``tidyr::spread()`](http://www.rdocumentation.org/packages/tidyr/functions/spread).
