...
- Execute the `ggplot2` code, and see if you can understand what each part does (we will go through each part in depth next)
...
*** =sct
```{r}
success_msg("You should see the plot appear beside the script in your window. Now you will learn the grammar of `ggplot2`")
```
...
<!-- TODO: This needs to be more clear. + combines elements. Remove adding at the end since it's confusing -->
...
success_msg("Well done! As the number of cylinders increases, what is the trend in horsepower (`hp`) and miles per gallon (`mpg`)?")
```
...
Remember, a `ggplot` is constructed by taking data, and *adding geometries*, layering our plot from bottom to top.
...
test_ggplot(check_data = TRUE, data_fail_msg = "Do not change the data argument.",
            ...
            check_geom = TRUE, aes_fail_msg = "Did you add a `geom_boxplot()` layer?")            
...
test_ggplot(check_data = TRUE, data_fail_msg = "Do not change the data argument.",
            check_aes = TRUE, aes_fail_msg = "Do not change the aesthetics.",
            check_geom = TRUE, aes_fail_msg = "Did you reorder the `geom` layers?")
...
test_ggplot(index = 2,
            ...
            check_geom = TRUE, aes_fail_msg = "Do not add/remove `geom` layers.")
success_msg("Well done!")
```
