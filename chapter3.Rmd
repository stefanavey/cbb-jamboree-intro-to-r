...
`ggplot2` is an R package that allows us to show off the inner beauty of data (after we tidy it up of course!). However, `ggplot2` is quite different from base R graphics - however, while there is a steeper learning curve, we believe you'll come to love `ggplot`. The reason? `ggplot2` allows us to both rapidly *explore* multifaceted datasets, as well as create *publication quality* plots to share with others, with a minimal amount of code (say goodbye to for-loops calling your plot functions!).

Let's start by taking a quick look at a built-in dataset, `mtcars`, a dataset that contains information for 32 cars from motor trends magazine from 1973. This dataset contains a variety of *categorical* (factors) and *continuous* variables for us to play with. 
...
- Explore the `mtcars` dataset with `head()` and `str()` to get a feel for the dataset
- Execute the `ggplot` code, and see if you can understand what each part does
...
# Run this code to produce your first ggplot
...
These are the basic ingredients of creating a plot in `ggplot2`. So, putting it all together, we get:

`data + geometry + aesthetic mappings + scaling`
...
*** =instructions
- Wrap the `factor()` function around `cyl` in your ggplot2 call
- Hit submit answer to see how the plot changes

*** =hint
Change `x = cyl` to `x = factor(cyl)` to alter the x-axis, and submit answer.
...
*** =instructions
Add an aesthetic, `size`, which maps to the variable `hp`, to the current `aes()` function, and check out the resulting bubble chart. How does `hp` correlate with `cyl` and `mpg`?
...
If we look at the [`geom_point()` docs](http://docs.ggplot2.org/current/geom_point.html), you might notice that in the **Aesthetics** section there is a variable `alpha` which controls transparency. These aesthetics can either be set to *variables*, as we have done with our `x`, `y`, and `size` aesthetics, but they can also be set to a *constant*, such that it *does not* change with the data point's values. 

Additionally, we want to keep our change alpha only within the `geom_point()` layer (e.g., we only want to alter the transparency of the *points*). To do that, we change alpha by setting it *within* the `geom_point()` function. 


*** =instructions
Change the alpha parameter such that `alpha = 0.5`. Not that alpha values range from {0, 1}, where 0 is transparent and 1 is opaque. By default, all points are opaque (`alpha = 1`).
...
Remember, a `ggplot` is constructed by taking data, and *adding geometries*, layering our plot from bottom to top. Just like how we added the *geom_point()* , add the *geom_boxplot()* as a layer to our plot.
...
*** =hint
Just like how we added the `geom_point()` layer, add the `geom_boxplot()` by using addition (`+`), s.t. we have `geom_point() + geom_boxplot()`. Note that is is common to have different layers on different lines, with the `+` indicating a continuing `ggplot` call.
...
Let's change our `ggplot` code to reflect a reordering of our layers (and change the transparency of the boxplots while we're at it!).
...
- Make the boxplots transparent, similar to how the points were made transparent

*** =hint
Our final function call should look something like: `ggplot(data, aes(...)) + geom_boxplot(alpha = 0.5) + geom_point(alpha = 0.5)`. 
...
To color *just the boxplots*, let's add a new `aes()` call *within* the `geom_boxplot()` layer, and instead of adding *color* at the top, add it to the `geom_boxplot(aes(...))` call.
